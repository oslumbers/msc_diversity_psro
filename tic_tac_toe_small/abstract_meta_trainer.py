...
def sample_episode(game, policies):
  """Samples an episode according to the policies, starting from state.
  Args:
    state: Pyspiel state representing the current state.
    policies: List of policy representing the policy executed by each player.
  ...
  """
  if game.is_terminal():
    return game.returns().astype(np.float32)

  player = game.current_player()
  a = policies[player].action((game.board2key()))
  game.update(a, player)

  return sample_episode(game, policies)
...
def nash_strategy(solver):
  ...
  metagame = nash.Game(*meta_games)
  probs = list(metagame.fictitious_play(iterations=5000))
  nash_prob_1 = probs[-1][0]
  nash_prob_2 = probs[-1][1]
