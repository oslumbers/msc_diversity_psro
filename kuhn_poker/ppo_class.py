...
from kuhn_poker.kuhn_env import infostate2vector, infostates
...
class PPOTrainer:
    ...
    def __init__(self, env, agent1, agent2, kuhn_pop, seed=0, lambda_weight=1., dpp=False,
    ...
    def compute_loss_pi(self, data):
        ...
        if self.dpp:
            prob_agents = []
            for agent in self.kuhn_pop.pop:
                prob_agents.append(agent.get_prob_allinfostates())
            X = torch.stack(prob_agents, 0)  # We add the training strategy to the list of pop
            X = F.normalize(X, dim=1, p=2)  # Normalise
            L = X @ X.t()  # Compute kernel
    ...
    def update(self, switch=False):
        ...
        for i in range(self.train_pi_iters):
            ...
            loss_pi, L_card, pi_info = self.compute_loss_pi(data)
            kl = pi_info['kl']
            if kl > 1.5 * self.target_kl:
                break
            if switch:
                loss = self.lambda_weight*loss_pi + (1.-self.lambda_weight)*L_card
            else:
                loss = self.lambda_weight * loss_pi - (1. - self.lambda_weight) * L_card
            loss.backward()
    ...
    def train(self, switch=False):
        ...
        L_card = self.update(switch=switch)
...
class PPO:
    def __init__(self, env,
                 ...
                 gamma=0.99, lam=0.97, pi_lr=3e-4, vf_lr=1e-3, target_kl=0.01, clip_ratio=0.2,
